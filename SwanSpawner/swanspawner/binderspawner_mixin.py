##from https://github.com/jupyterhub/binderhub/blob/d5d26aad23a6a943b753f4eca556a4f84d3bcf50/binderhub/binderspawner_mixin.py
    ...
    def get_args(self):
        if self.auth_enabled:
            ...
            args = [
                ...
                f"--NotebookApp.token={os.environ['JUPYTERHUB_API_TOKEN']}",
    ...
    def start(self):
        if not self.auth_enabled:
            if 'token' not in self.user_options:
                self.log.warning("no token is set")
                #raise web.HTTPError(400, "token required")
            if 'image' not in self.user_options:
                self.log.warning("no image is set")
